...
class MainApp extends Component {
  ...
  state = {
    paintings,
    message: "Click on any color card below to copy the palette, declared as SCSS variables, to your clipboard! ",
    copied: false,
  };
        ...
        <div id="main">
          ...
          <div className="container">
            ...
            {this.state.paintings.map(palette => (
              <div className="paletteCard col-10 col-md-6 col-xl-4 my-4 mx-auto" key={palette.id} onClick={this.copyToClipboard}>
                <div className="paletteCardInside">
                  <h5 id={palette.name} className="cardHeader mb-0">{palette.name}</h5> 
                  {palette.colorArr.map(color => {
                    const colorBarStyle = {backgroundColor: color};
                    return <div className="colorBar" style={colorBarStyle} key={color}><p className="colorText">{color}</p></div>
                  })}
                  <CopyToClipboard text={palette.clipBoard}
                    onCopy={() => this.setState({message: 'Palette SCSS Variables Copied to Clipboard'})}>
                    <button className="my-3 copyBtn">Copy <i class="far fa-clipboard"></i></button>
                  </CopyToClipboard>
